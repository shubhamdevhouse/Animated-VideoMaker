...
processVideo = async function (blockJsonObj, blockDataObj, id, i) {
    ...
    new Promise((resolve, reject) => {
        ...
        port.on('message', (data) => {
            ...
            if (typeof (users[data.id]["blockList"][data.index]) === "undefined") {
                let obj = new Object();
                obj.socket = users[data.id].socket;
                if (!users[data.id].isPreview)
                    obj.data = {
                        'status': 'error', videos_progress: { '360p': 0, '480p': 0, '720p': 0, '1080p': 0 }
                        , videos_url: { '360p': 'none', '480p': 'none', '720p': 'none', '1080p': 'none' }
                    }
                ...
                users[data.id].eventEmitter.emit('onProgress', obj);
                return;
            } else {
                users[data.id]["blockList"][data.index].url = outputFile;
                if (users[id].commands.length > 0 && 
                    this.currentIndex >= users[id].commands.length && users[id].totalVideos == users[id].processedVideos) {
                    this.processHDVideo(users[ide].socket, id, users[id].p_360, users[id].p_480, users[id].p_720, users[id].p_1080)
                }
            }
        });
...
processImage = async function (blockDataObj, id, i,j,animation_style) {
   ...
   */
   ...
   if(users[id].blocksData[i].type!="photo burst"){
    ...
    //console.log("convert", blockDataObj.url, ...additional_cmd, "-resize", (blockDataObj.background_settings.scale * 100) + "%", "-rotate", blockDataObj.background_settings.rotate, ...extent_cmd, Config.output_directory + id +  path.sep +"data" +path.sep+ "p" + path.basename(blockDataObj.parsedMedia.pathname));
    await new Promise((resolve, reject) => {
        let outputFile = Config.output_directory + id +  path.sep +"data" +path.sep+ "p" +Utils.generateRandomID()+ path.basename(blockDataObj.parsedMedia.pathname);
        let port = null;
        port = new Worker(('./workerChild.js'), {
...
//Text functions
getTextDimension = async function (height, text, fontSize, fontCurrent) {
    let origText = text;
    let res = "";
    await Utils.runProcess("convert",
        ["convert", "-size", Config.hdWidth+"x" + height, "xc:lightblue", "-font", fontCurrent
            , "-pointsize", fontSize, , "-strokewidth", 2, "-fill", "black", "-undercolor", "none",
            "-annotate", "+0+" + parseInt(height / 2), origText, "-trim", "info:"]).then(resData => {
                res = resData

            });
    let widthSub = parseInt(Utils.getDataInBetween(res,"xc:lightblue XC ","x"));

    let ascent = parseInt(Utils.getDataInBetween(res,"xc:lightblue XC " + widthSub + "x"," "));
    let iFrom = Utils.getDataStartPoint(res,Config.hdWidth+"x" + height)+1;
    let iTo = Utils.getDataEndPoint(res,"+",false,iFrom+1);

    let nextCommand = Config.hdWidth+"x" + height + "+"
    if (iTo == -1) {
        iTo = parseInt(Utils.getDataEndPoint(res,"-",false,iFrom+1));
        nextCommand = Config.hdWidth+"x" + height + "-";
    }
    
    let xAxis = parseInt(Utils.getDataInBetweenInt(res,iFrom,iTo- iFrom));
    
    iFrom = Utils.getDataStartPoint(res,nextCommand + xAxis)+1;
    iTo = Utils.getDataEndPoint(res," ",false,iFrom+1);

    let tempDescent = parseInt(Utils.getDataInBetweenInt(res,iFrom,iTo- iFrom));
    let tempAscent = parseInt(ascent)
    
    let finalAscent = (parseInt(height / 2)) - tempDescent

    let finalDescent = parseInt(tempAscent - finalAscent);

    let dimen = await this.getWordDimension(text, fontSize, fontCurrent)

    let dimension = { "widthSub": dimen["widthSub"], "descent": finalDescent, "ascent": finalAscent }
    return dimension
}

getWordDimension = async function (text, fontSize, fontCurrent) {
    let origText = text;
    let res = "";
    await Utils.runProcess("convert",
        ["convert", "-debug", "annotate", "xc:", "-font", fontCurrent
            , "-pointsize", fontSize, , "-strokewidth", 2,
            "-annotate", "+0+0", origText, "null:"]).then(resData => {
                res = resData

            }); 
    let widthSub = parseInt(Utils.getDataInBetween(res,"width: ","; height"));
    let heightSub = parseInt(Utils.getDataInBetween(res,"ascent: ","; descent"));
    let descent = parseInt(Utils.getDataInBetween(res,"descent: ","; max adv"));
    let advanceHeight = parseInt(Utils.getDataInBetween(res,"max advance: ","; bounds"));
    let totalHeight = parseInt(Utils.getDataInBetween(res,"height: ","; ascent"));
    

    let result  = Utils.getDataInBetween(res,"bounds: ","; origin");
    let totalBound = 0
    if (result.length <= 0) {
        totalBound = 0
    } else {

        totalBound = parseInt(result.split(',')[1].split(' ')[0])
    }
    let dimension = { "widthSub": widthSub, "descent": descent, "heightSub": heightSub, "totalBound": totalBound, "totalHeight": totalHeight,"advanceHeight":advanceHeight }
    return dimension
}

getTextDimensions = async function (fontSize, text, id, blockIndex, linesList, fontCurrent, type) {
    let users= Config.users;
    linesList["text"] = []
    linesList["height"] = []
    linesList["width"] = [];
    linesList["descent"] = [];
    linesList["totalHeight"] = [];
    linesList["advanceHeight"] = [];

    linesList["heightSub"] = [];
    linesList["totalBound"] = [];
    let origText = text
    if (users[id]["blockList"][blockIndex].type == "quoteImage" || users[id]["blockList"][blockIndex].type == "quoteVideo") {

        origText = ("“" + origText.trim() + "„");
    }

    let resultDimension = await this.getWordDimension(origText.trim(), fontSize, fontCurrent)
    ...
    if (resultDimension["widthSub"] < maxWidthLocal - users[id].maxFirstCrop && !iamDone) {
        if (resultDimension["widthSub"] % 2 == 0) {
            linesList["width"].push(resultDimension["widthSub"]);
        }
        else {
            linesList["width"].push(resultDimension["widthSub"] + 1);
        }

        linesList["totalHeight"].push(resultDimension["totalHeight"]);
        linesList["heightSub"].push(resultDimension["heightSub"]);
        linesList["advanceHeight"].push(resultDimension["advanceHeight"]);
        
        linesList["totalBound"].push(parseInt(resultDimension["totalBound"]));


        linesList["descent"].push(resultDimension["descent"]);
        linesList["height"].push(resultDimension["heightSub"]);
        linesList["text"].push(origText.trim());
    ...
    this.currentIndex = -1;
    let splittedText = origText.split(' ')
    this.currentIndex = 0
    let nOrigText = "";
    if (iamDone) { return; }
    while (!iamDone) {
        ...
        nOrigText = splittedText[this.currentIndex].trim();
        if (nOrigText.length <= 0) {
            this.currentIndex++
            nOrigText = splittedText[this.currentIndex].trim();
        }
    
        resultDimension = await this.getWordDimension(nOrigText, fontSize, fontCurrent);

        if (this.currentIndex == 0 && resultDimension["widthSub"] > maxWidthLocal - users[id].maxFirstCrop
        && resultDimension["heightSub"] > users[id].maxHeightBound - users[id].maxHeightStart) {
            let secondLoop = true;
            while (!secondLoop) {
                currentCharIndex = nOrigText.length - 1;
                nOrigText = nOrigText.substr(0, currentCharIndex);
                resultDimension = await this.getWordDimension(nOrigText, fontSize, fontCurrent);
                if (this.currentIndex == 0 && resultDimension["widthSub"] <= maxWidthLocal - users[id].maxFirstCrop && resultDimension["heightSub"] <= users[id].maxHeightBound - users[id].maxHeightStart) {
                    if (resultDimension["widthSub"] % 2 == 0) {
                        linesList["width"].push(resultDimension["widthSub"]);
                    ...
                    linesList["descent"].push(resultDimension["descent"]);
                    linesList["height"].push(resultDimension["heightSub"]);
                    linesList["text"].push(nOrigText.trim());

                    secondLoop = true
                }

            }
        } else if (this.currentIndex != -1 && resultDimension["widthSub"] > maxWidthLocal - users[id].maxFirstCrop) {
            let temp = nOrigText;
            let widthSub = resultDimension["widthSub"];
            while (widthSub > maxWidthLocal - users[id].maxFirstCrop) {
                currentCharIndex = temp.length - 1;
                temp = temp.substr(0, currentCharIndex);
                resultDimension = await this.getWordDimension(temp, fontSize, fontCurrent);
                widthSub = resultDimension["widthSub"];
                if (this.currentIndex > -1 && resultDimension["widthSub"] < users[id].maxWidthBound - users[id].maxFirstCrop) {
                    splittedText[this.currentIndex] = temp;
                    temp = nOrigText.substr(currentCharIndex, nOrigText.length);
                    splittedText.splice(this.currentIndex + 1, 0, temp);
                    if (resultDimension["widthSub"] % 2 == 0) {
                        linesList["width"].push(resultDimension["widthSub"]);
                    }
                    else {
                    ...
                    linesList["descent"].push(resultDimension["descent"]);
                    linesList["height"].push(resultDimension["heightSub"]);
                    linesList["text"].push(splittedText[this.currentIndex].trim());
                    this.currentIndex++;
                }
            }
        } else if (this.currentIndex > -1 && resultDimension["widthSub"] <= maxWidthLocal - users[id].maxFirstCrop) {
...
doProcessText = async function (id, blockIndex, animation_style) {
    ...
    await this.getTextDimensions(users[id]["blockList"][blockIndex].text[0]["fontSize"], users[id]["blockList"][blockIndex].text[0]["text"].trim(), id, blockIndex, users[id]["blockList"][blockIndex].wordsListMain, this.fontBold, "Main");
    ...
    for (let i = 0; i < users[id]["blockList"][blockIndex].wordsListMain["text"].length; i++) {
        tempWidth += users[id]["blockList"][blockIndex].wordsListMain["width"][i];
        
        tempText += users[id]["blockList"][blockIndex].wordsListMain["text"][i] + " ";
        let inc = i + 1;
        if (
            tempWidth + users[id]["blockList"][blockIndex].wordsListMain["width"][inc]
            >
            users[id].maxWidthBound - (users[id].maxFirstCrop * 2) && inc < users[id]["blockList"][blockIndex].wordsListMain["text"].length) {
            if (typeof (tempObject["textLine"]) === "undefined") {
                tempObject["widthLine"] = tempWidth;
                tempObject["heightLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListMain["totalHeight"][0]);
                tempObject["advanceLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListMain["advanceHeight"][0]);
                tempObject["nDescentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListMain["heightSub"][0]);
                tempObject["descentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListMain["descent"][0]);
                tempObject["ascentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListMain["height"][0]);
                tempObject["textLine"] = tempText;
            }
            let obj = await this.getTextDimension(tempObject["heightLine"] * 2, tempText.trim(), users[id]["blockList"][blockIndex].text[0]["fontSize"], this.fontBold)
            tempObject['ascentLine'] = parseInt(obj["ascent"]);
            tempObject['widthLine'] = parseInt(obj["widthSub"]);
            tempObject["descentLine"] = parseInt(obj["descent"]);
            tempObject["textLine"] = tempText.trim();
            ...
            tempObject["ascentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListMain["height"][0]);
            tempObject["textLine"] = tempText;
            if (i == users[id]["blockList"][blockIndex].wordsListMain["text"].length - 1) {
                tempObject["textLine"] = tempObject["textLine"].trim();
                //let obj = await getWordDimension(tempObject["textLine"],users[id]["blockList"][blockIndex].text[0]["fontSize"],fontBold)
                let obj = await this.getTextDimension(tempObject["heightLine"] * 2, tempObject["textLine"], users[id]["blockList"][blockIndex].text[0]["fontSize"],this.fontBold);
                tempObject['ascentLine'] = parseInt(obj["ascent"]);
                tempObject['widthLine'] = obj["widthSub"];
                tempObject["descentLine"] = parseInt(obj["descent"])
                users[id]["blockList"][blockIndex].linesListMain.push(tempObject);
                tempText = "";
                tempWidth = 0;
    ...
    await this.getTextDimensions(users[id]["blockList"][blockIndex].text[1]["fontSize"], users[id]["blockList"][blockIndex].text[1]["text"].trim(), id, blockIndex, users[id]["blockList"][blockIndex].wordsListSub, this.subFont, "Sub");
    tempWidth = 0;
    tempText = "";
    tempObject = {};
    let maxWidthBound = users[id].maxWidthBound;
    if (users[id]["blockList"][blockIndex].alignH == "center") {
        maxWidthBound = users[id].maxWidthBound - ((maxWidthBound / 2) - (
            users[id]["blockList"][blockIndex].linesWidthMain[0] / 2)) - (60);
    }
    for (let i = 0; i < users[id]["blockList"][blockIndex].wordsListSub["text"].length; i++) {
        tempWidth += users[id]["blockList"][blockIndex].wordsListSub["width"][i];
        tempText += users[id]["blockList"][blockIndex].wordsListSub["text"][i] + " ";
        let inc = i + 1;
        if (
            tempWidth + users[id]["blockList"][blockIndex].wordsListSub["width"][inc]
            >
            maxWidthBound - (users[id].maxFirstCrop * 2) && inc < users[id]["blockList"][blockIndex].wordsListSub["text"].length) {

            if (typeof (tempObject["textLine"]) === "undefined") {
                tempObject["widthLine"] = tempWidth;
                tempObject["heightLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListSub["totalHeight"][0]);
                tempObject["advanceLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListSub["advanceHeight"][0]);
                tempObject["nDescentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListSub["heightSub"][0]);
                tempObject["descentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListSub["descent"][0]);
                tempObject["ascentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListSub["height"][0]);
                tempObject["textLine"] = tempText;
            }
            let obj = await this.getTextDimension(tempObject["heightLine"] * 2, tempText.trim(), users[id]["blockList"][blockIndex].text[1]["fontSize"],this. subFont)
            tempObject['ascentLine'] = parseInt(obj["ascent"]);
            tempObject['widthLine'] = parseInt(obj["widthSub"]);
            tempObject["descentLine"] = parseInt(obj["descent"]);
            tempObject["textLine"] = tempText.trim();
            ...
            tempObject["ascentLine"] = parseInt(users[id]["blockList"][blockIndex].wordsListSub["height"][0]);
            tempObject["textLine"] = tempText;
            if (i == users[id]["blockList"][blockIndex].wordsListSub["text"].length - 1) {
                tempObject["textLine"] = tempObject["textLine"].trim();
                let obj = await this.getTextDimension(tempObject["heightLine"] * 2, tempObject["textLine"], users[id]["blockList"][blockIndex].text[1]["fontSize"], this.subFont);
                tempObject['ascentLine'] = parseInt(obj["ascent"]);
                tempObject['widthLine'] = obj["widthSub"];
                tempObject["descentLine"] = parseInt(obj["descent"]);
                users[id]["blockList"][blockIndex].linesListSub.push(tempObject);
...
doAlign = function (user,animation_style,maxHeightBound, maxFirst, maxWidthBound, maxWidthBoundCrop, maxFirstCrop, maxHeightStart, blockIndex, linesList, linesListxAxis, linesListyAxis, linesWidth) {
    ...
    for (let gf = 0; gf < linesList.length; gf++) {
        ...
        //console.log(user["blockList"][blockIndex].alignH ,"|||||||||||")
        let finalX = 0
